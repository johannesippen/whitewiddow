  ...
  friendQuery.find(function(friendQueryResult) {
    ...
    if (friendQueryResult.length > 0) {
      ...
      userQuery.find(function(userQueryResult) {
        ...
        var middlePromise = loadFoursquareData(middle, 500, "middle", venueResult, userLocation);
        var friendPromise = loadFoursquareData(friendMiddle, 500, "friend", venueResult, userLocation);
        var userPromise = loadFoursquareData(userMiddle, 500, "mine", venueResult, userLocation);
  ...
  function loadFoursquareData(location, radius, type, venueList, userLoc) {
    ...
    return Parse.Cloud.httpRequest({
      ...
      success: function(httpResponse) {
        ...
        if (!bestVenue) {
          // FIXME: Isn't this https://trello.com/c/Vr2yCbfS/197-radius-extention-if-no-venue-is-available already?
          radius = radius * 2;
          ...
          lookupQuery.find(function(lookupResult) {
            ...
            if (lookupResult.length == 0) {
              ...
              var point = new Parse.GeoPoint({
                latitude: bestVenue["location"]["lat"],
                longitude: bestVenue["location"]["lng"]
              });
      ...
      error: function(httpResponse) {
        ...
        // FIXME: Alternative: Provide Static Location here (Mein Haus am See). Cheap, but does the trick
